...
unsigned long long variable_sources_rdtsc = 0;
unsigned long long variable_sources_else_rdtsc = 0;
unsigned long long gfs_domain_bc_rdtsc = 0;
unsigned long long gfs_domain_traverse_leaves_rdtsc = 0;

unsigned long long b_rdtsc_t, a_rdtsc_t;

double  gfs_domain_traverse_leaves_clk = 0.0;
double  variable_sources_clk = 0.0;
double  gfs_domain_variable_centered_sources_clk = 0.0;
double  gfs_domain_cell_traverse_var_src_clk = 0.0;
double  gfs_velocity_face_sources_clk = 0.0;
double  gfs_poisson_coefficients_mac_clk = 0.0;
double  poisson_solve_mac_clk = 0.0;
double  gfs_domain_face_traverse_approx1_clk = 0.0;
double  gfs_domain_face_traverse_approx2_clk = 0.0;
			    ...
			    )
{
  ...
  gfs_velocity_face_sources_clk = gfs_velocity_face_sources_clk +( stop.tv_sec - start.tv_sec )
  ...
  gfs_poisson_coefficients_mac_clk =   gfs_poisson_coefficients_mac_clk +( stop.tv_sec - start.tv_sec )
          + ( stop.tv_nsec - start.tv_nsec )/ BILLION;
  ...
  poisson_solve_mac_clk = poisson_solve_mac_clk +( stop.tv_sec - start.tv_sec )
          + ( stop.tv_nsec - start.tv_nsec )/ BILLION;
				 ...
				 )
{
  ...
  gfs_domain_face_traverse_approx1_clk =  gfs_domain_face_traverse_approx1_clk +( stop.tv_sec - start.tv_sec )
          + ( stop.tv_nsec - start.tv_nsec )/ BILLION;
  if( clock_gettime( CLOCK_REALTIME, &start) == -1 ) {
      perror( "clock gettime" );
       exit( EXIT_FAILURE );
  ...
  gfs_domain_face_traverse_approx2_clk =  gfs_domain_face_traverse_approx2_clk +( stop.tv_sec - start.tv_sec )
          + ( stop.tv_nsec - start.tv_nsec )/ BILLION;
			      ...
			      GfsVariable ** g)
{
  ...
  if (g) {
    ...
    gfs_domain_cell_traverse_var_src_clk =   gfs_domain_cell_traverse_var_src_clk +( stop.tv_sec - start.tv_sec )
          + ( stop.tv_nsec - start.tv_nsec )/ BILLION; 
  ...
  gfs_domain_variable_centered_sources_clk =  gfs_domain_variable_centered_sources_clk +( stop.tv_sec - start.tv_sec )
          + ( stop.tv_nsec - start.tv_nsec )/ BILLION;  
						...
						GfsFunction * alpha)
{
  ...
  for (c = 0; c < dimension; c++) {
    ...
    if (d) {
      ...
      gfs_domain_traverse_leaves_clk = gfs_domain_traverse_leaves_clk +( stop.tv_sec - start.tv_sec )
          + ( stop.tv_nsec - start.tv_nsec )/ BILLION;
      gfs_domain_traverse_leaves_rdtsc += b_rdtsc_t - a_rdtsc_t;
      ...
      variable_sources_clk = variable_sources_clk +( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/ (double)BILLION;
      b_rdtsc_t = rdtsc();  
      variable_sources_rdtsc += b_rdtsc_t - a_rdtsc_t;
      ...
      gts_object_destroy (GTS_OBJECT (rhs));
        ...
        variable_sources_else_rdtsc += b_rdtsc_t - a_rdtsc_t;
    }
   }
  ...
  for (c = 0; c < dimension; c++)
    gfs_domain_bc (domain, FTT_TRAVERSE_LEAFS, -1, v[c]);
  b_rdtsc_t = rdtsc();  
  gfs_domain_bc_rdtsc += b_rdtsc_t - a_rdtsc_t;

...
struct timespec startes, stopes;
struct timespec starteks, stopeks;

double get_from_above_clk = 0.0;
double relax_loop_clk = 0.0;
double gfs_traverse_and_homogeneous_bc_clk = 0.0;
double gfs_poisson_cycle_clk = 0.0;

unsigned long long gfs_poisson_cycle_rdtsc = 0.0;
unsigned long long get_from_above_rdtsc = 0.0;
unsigned long long relax_loop_rdtsc = 0.0;
unsigned long long gfs_traverse_and_homogeneous_bc_rdtsc = 0.0;

unsigned long long a_rdtsc_p, b_rdtsc_p = 0.0;
unsigned long long a_rdtsc_pc, b_rdtsc_pc = 0.0;
unsigned long long a_rdtsc_pc_in, b_rdtsc_pc_in = 0.0;
/**
			...
			FttCellTraverseFunc relaxfunc)
{
  ...
  a_rdtsc_pc_in = rdtsc();
  if( clock_gettime( CLOCK_MONOTONIC, &startes) == -1 ) {
  ...
  for (n = 0; n < nrelax - 1; n++)
   ...
   if( clock_gettime( CLOCK_MONOTONIC, &stopes) == -1 ) {
        perror( "clock gettime" );
        exit( EXIT_FAILURE );
   }   
   b_rdtsc_pc_in = rdtsc();
   gfs_traverse_and_homogeneous_bc_rdtsc += b_rdtsc_pc_in - a_rdtsc_pc_in; 
   gfs_traverse_and_homogeneous_bc_clk += (stopes.tv_sec - startes.tv_sec )
          + (double)( stopes.tv_nsec - startes.tv_nsec )/ (double)BILLION;
			...
			GfsVariable * res)
{
  ...
  for (q.maxlevel = minlevel + 1; q.maxlevel <= p->depth; q.maxlevel++, nrelax /= p->erelax) {
    ...
    b_rdtsc_pc = rdtsc();
    get_from_above_rdtsc += b_rdtsc_pc - a_rdtsc_pc;
    get_from_above_clk += (stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/ (double)BILLION;
 
   #ifdef CONTIGUOUS_ALLOCATION
    from_top_to_bottom_traverse = 0;
    #endif
    a_rdtsc_pc = rdtsc();
    ...
    b_rdtsc_pc = rdtsc();
    relax_loop_rdtsc +=  b_rdtsc_pc - a_rdtsc_pc;
    relax_loop_clk += (stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/ (double)BILLION;
  }
			...
			GfsVariable * dia, gdouble dt)
{
  ...
  while (par->niter < par->nitermin ||
    ...
    a_rdtsc_p = rdtsc();  
    /* Does one iteration */
    if( clock_gettime( CLOCK_MONOTONIC, &starteks) == -1 ) {
        perror( "clock gettime" );
        exit( EXIT_FAILURE );
    } 
    gfs_poisson_cycle (domain, par, lhs, rhs, dia, res);
    if( clock_gettime( CLOCK_MONOTONIC, &stopeks) == -1 ) {
         perror( "clock gettime" );
         exit( EXIT_FAILURE );
    }
    b_rdtsc_p = rdtsc();  
    gfs_poisson_cycle_rdtsc += b_rdtsc_p - a_rdtsc_p;

    gfs_poisson_cycle_clk += (stopeks.tv_sec - starteks.tv_sec )
          + (double)(stopeks.tv_nsec - starteks.tv_nsec )/ (double)BILLION;    

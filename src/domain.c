...
unsigned long long a_rdtsc = 0;  
unsigned long long b_rdtsc = 0;

double domain_foreach_clk = 0.0;
double domain_foreach_gpu_clk = 0.0;
double gfs_domain_cell_traverse_homogeneous_bc_clk = 0.0;
double gfs_homogeneous_bc_clk = 0.0;


unsigned long long  gfs_domain_foreach_gpu_rdtsc = 0;

#ifdef CONTIGUOUS_ALLOCATION
char from_top_to_bottom_traverse = 0;
#ifdef WAY1
gpointer cellData = NULL;
...
gpointer my_malloc_cell_data(unsigned long size)
{
        static gpointer cellDataEnd, nextCellDataFree;
        gpointer returnPointer;

	
	if (cellData == NULL) {
		cellData = g_malloc0(size);
		cellDataEnd = (gpointer)((unsigned long)cellData + size);
		nextCellDataFree = cellData;
                return cellData;
	}
	
	size += 100;
	//size = roundUp(size, 8);
	returnPointer = nextCellDataFree;
        nextCellDataFree = (gpointer)((unsigned long)nextCellDataFree + size);
	if ((unsigned long)nextCellDataFree >= (unsigned long)cellDataEnd) {
		printf("Cell Data: Memory allocation problem\n");
		exit(0);
	}
	return(returnPointer);
};
#elif defined(WAY3)
struct Data_Of_Cell * cell_data;
GfsSolidVector * solid_data;
int nxtIndxData = 0;
int nxtIndxSolidData = 0;

struct Data_Of_Cell * my_malloc_data_struct()
{
    struct Data_Of_Cell * returnPointer = &cell_data[nxtIndxData]; 
    nxtIndxData++;
    return returnPointer;
};

struct  GfsSolidVector * my_malloc_solid_data()
{
    struct GfsSolidVector * returnPointer = &solid_data[nxtIndxSolidData];
    nxtIndxSolidData;
    return returnPointer;
}
#endif
			    ...
			    gpointer data)
{
  ...
  if (a == NULL) /* domain is being destroyed */
    ...
    for (i = 0; i < a->len; i++)
      (* func) (a->pdata[i], data);
    #else
    if (from_top_to_bottom_traverse == 1) {
      ...
      // checkErr(err, "cell_data WRITE BUFFER ERROR"); 
       for (i = 0; i < a->len; i++) {
        /*GfsBox * box = a->pdata[i];
        printf("Root[%d] is %p,  a->pdata[%d]->root is %p\n", i, (struct _FttRootCell *)Root + i, i, box->root);*/
         runCLTraverseTreePerLevel(d->max_depth, d->data, i);  
       }

     //  err = clEnqueueReadBuffer(commands, cellData_d, CL_TRUE, 0, 30000 * (sizeof(struct Data_Of_Cell) + sizeof (GfsSolidVector)), cellData, 0, NULL, NULL);
     //  checkErr(err, "cell_data READ BUFFER ERROR:");
       ...
       domain_foreach_gpu_clk += (stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/ (double)BILLION;

       b_rdtsc = rdtsc();
       gfs_domain_foreach_gpu_rdtsc += b_rdtsc - a_rdtsc;
       ...
       domain_foreach_clk += (stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/ (double)BILLION;
    }
    else
       for (i = 0; i < a->len; i++)
				      ...
				      GfsVariable * v)
{
  ...
  if (domain->pid < 0 || !domain->overlap) {
    ...
    gfs_domain_cell_traverse_homogeneous_bc_clk +=( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/(double) BILLION;
    ...
    gfs_homogeneous_bc_clk +=( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/(double) BILLION;
  }
  else {
    TraverseBcData d = {
...
void gfs_cell_init (FttCell * cell, GfsDomain * domain)
{
  ...
  if (FTT_CELL_IS_LEAF (cell)) {
    ...
    #else
     #ifdef WAY1
      cell->data = my_malloc_cell_data(gfs_domain_variables_size (domain));
     #elif defined(WAY3)
     cell->data = my_malloc_data_struct();
     #endif 
    #endif
  }
  else {
    ...
    for (n = 0; n < FTT_CELLS; n++) {
      ...
      #else
       #ifdef WAY1
       child.c[n]->data = my_malloc_cell_data(gfs_domain_variables_size (domain));
       #elif defined(WAY3)
       child.c[n]->data = my_malloc_data_struct();
       #endif
      #endif
    }
...
void gfs_cell_read (FttCell * cell, GtsFile * fp, GfsDomain * domain)
{
  ...
  if (s0 >= 0.) {
    ...
    #ifndef CONTIGUOUS_ALLOCATION 
    printf("gfs_cell_read: solid ordinary way\n");
    s->solid = g_malloc0 (sizeof (GfsSolidVector));
    #else
     #if defined(WAY1)
      printf("gfs_cell_read: solid way1\n");
      s->solid = my_malloc_cell_data(sizeof (GfsSolidVector));
     #elif defined(WAY3)
      printf("gfs_cell_read: solid way3\n");
      s->solid = my_malloc_solid_data();
     #endif  
    #endif
...
void gfs_cell_read_binary (FttCell * cell, GtsFile * fp, GfsDomain * domain)
{
  ...
  if (s0 >= 0.) {
    ...
    #ifndef CONTIGUOUS_ALLOCATION 
    printf("gfs_cell_read_binary: solid ordinary way\n");
    s->solid = g_malloc0 (sizeof (GfsSolidVector));
    #else
     #if defined(WAY1)
      printf("gfs_cell_read_binary: solid way1\n");
      s->solid = my_malloc_cell_data(sizeof (GfsSolidVector));
     #elif defined(WAY3)
      printf("gfs_cell_read_binary: solid way3\n");
      s->solid = my_malloc_solid_data();
     #endif  
    #endif

...
gpointer treeEnd, tree;
cl_context context;
cl_command_queue commands;
cl_program program;  
cl_kernel convert_tree_address_kernel;
cl_kernel get_ptr_kernel;
cl_kernel convertTreeAddresses_kernel;
...
FttCell * check_who_is_parent(FttCell *parent)
{
	   if( (unsigned long)Root <= (unsigned long)parent  && (unsigned long)parent <= (unsigned long)RootEnd)
	        return Root;
	   else if((unsigned long)tree <= (unsigned long)parent && (unsigned long)parent <= (unsigned long)treeEnd)
	        return tree;
	   else 
	        return NULL; 
}

gpointer my_malloc_tree(unsigned long size)
{
        static gpointer  nextTreeFree;
	gpointer returnPointer;

	if (tree == NULL) {
		tree = g_malloc0(size);
		treeEnd = (gpointer)((unsigned long)tree + size);
		nextTreeFree = tree;
                return NULL;
	}
	
	returnPointer = nextTreeFree;
	nextTreeFree = (gpointer)((unsigned long)nextTreeFree + size);
	if ((unsigned long)nextTreeFree >= (unsigned long)treeEnd) {
		printf("Tree: Memory allocation problem\n");
		exit(0);
	}
			...
			case CL_INVALID_GLOBAL_WORK_SIZE:							
				...
				break;
                        ...
                        case CL_OUT_OF_RESOURCES:
                                printf("CL_DEVICE_OUT_OF_RESOURCES\n");
                                break;
                        ...
                        case CL_OUT_OF_HOST_MEMORY:
                                printf("CL_INVALID_OUT_OF_HOST_MEMORY\n");
                                break;
...
/***************************************
*Choosing plaform and creating a context
****************************************/
cl_context createContext()
{
	...
	err = clGetPlatformIDs(0, NULL , &numPlatforms);
        ...
        for(i = 0; i < numPlatforms; i++) {
         err = clGetPlatformInfo(platformIds[i], CL_PLATFORM_NAME, 0, NULL, &size);
         name = (char *)malloc(sizeof(char) * size);
         err = clGetPlatformInfo(platformIds[i], CL_PLATFORM_NAME, size, name, NULL);

         err = clGetPlatformInfo(platformIds[i], CL_PLATFORM_VENDOR, 0, NULL, &size);
         vname = (char *)malloc(sizeof(char) * size);
         err = clGetPlatformInfo(platformIds[i], CL_PLATFORM_VENDOR, size, vname, NULL);
         printf("platformIds[%d] is %s and Vendor name is %s\n", i,  name, vname);
        }
	...
	cl_device_id *devices = NULL;
        ...
        err = clGetDeviceIDs(platformIds[0], CL_DEVICE_TYPE_ALL, 0, NULL, &numDevices);
        printf("numDevices Platform AMD is %d\n", numDevices);
       
        /*err = clGetDeviceIDs(platformIds[1], CL_DEVICE_TYPE_GPU, 0, NULL, &numDevices);       
        printf("numDevices Platform NVIDIA is %d\n", numDevices);*/

	devices = (cl_device_id *)malloc(numDevices * sizeof(cl_device_id));

	err = clGetDeviceIDs(platformIds[0], CL_DEVICE_TYPE_ALL, numDevices, devices, NULL);

        //Query the properties of the devices
	char param_value1[60];
        
        for(i = 0; i < numDevices; i++) {
	  clGetDeviceInfo (devices[i], CL_DEVICE_NAME, sizeof(param_value1), param_value1, NULL);
	  printf("DEVICE_NAME devices[%d]: %s\n", i, param_value1);
	}
	
        //Query the properties of the devices
	context1 = NULL;
	context1 = clCreateContext(NULL, 1, &devices[0], NULL, NULL, &err);
...
cl_command_queue createCommandQueue(cl_context context, cl_device_id *device)
{
	...
	err = clGetContextInfo(context, CL_CONTEXT_DEVICES, 0, NULL, &devicesBufferSize);
        checkErr(err,"Get context info\n");
	...
	devices = (cl_device_id *)malloc(devicesBufferSize * sizeof(cl_device_id));
	err = clGetContextInfo(context, CL_CONTEXT_DEVICES, devicesBufferSize, devices, NULL);
	checkErr(err,"Failed to get devices ids\n");

        char param_value1[60];
        printf("devices buffer size is %d\n", devicesBufferSize);	
        clGetDeviceInfo (devices[0], CL_DEVICE_NAME, sizeof(param_value1), param_value1, NULL);
	printf("DEVICE_NAME devices[0]: %s\n", param_value1);
        
       	commandQueue = clCreateCommandQueue(context, devices[0], CL_QUEUE_PROFILING_ENABLE, &err);
	if(commandQueue == NULL) {
	 printf("Failed to create command queue for GTX 480");
 	}
        checkErr(err,"Create Command Queue\n");
...
cl_program createProgram(cl_context context, cl_device_id device, const char * name)
{
	...
	program_tmp = clCreateProgramWithSource(context, 1, (const char **)&source_str, NULL, &err);
        if(program_tmp == NULL)
          printf("FAILED to create the program\n");
	
        checkErr(err,"PROGRAM FROM SOURCE ERROR");
 
        int numDevices = 1;

        char options[30];
        strcpy(options, "-g");
	err =  clBuildProgram(program_tmp, numDevices, &device, NULL, NULL, NULL);   
	if(err !=CL_SUCCESS) {
		size_t len;
		char buffer[30000];
		printf("error to build program executable\n");
		clGetProgramBuildInfo(program_tmp, device, CL_PROGRAM_BUILD_LOG, sizeof(buffer), buffer, &len);
		printf("%s\n", buffer);	
                clReleaseProgram(program_tmp);
		return NULL;	
 	}
        checkErr(err, "PROGRAM ERROR");
...
/*
*runCLInit:
*@filename:file's name containing the OpenCL kernels
*
*Discovers and initializes the platform and the devices
*Creates a context, a command queue, the program, and the kernels
*Compiles the program
*/
int runCLinit(char * filename)
{
        cl_int err;
        cl_device_id device;                    
	...
	context = createContext();
        ...
        commands = NULL;
        commands = createCommandQueue(context, &device);
        if(commands == NULL) {
	...
	if(program == NULL) {
	  printf("program is NULL"); 	
        ...
        cell_traverse_level_non_leafs_kernel = NULL;
        cell_traverse_level_non_leafs_kernel = clCreateKernel(program, "cell_traverse_level_non_leafs_kernel_func", &err); 
        if(cell_traverse_level_non_leafs_kernel == NULL)
           printf("cell_traverse_level_non_leafs_kernel is NULL\n");
        checkErr(err, "cell_traverse_level_non_leafs_kernel ERROR");

        convertTreeAddresses_kernel = NULL;
        convertTreeAddresses_kernel = clCreateKernel(program, "convertTreeAddresses", &err); 
        if(convertTreeAddresses_kernel == NULL)
           printf("convertTreeAddresses is NULL\n");
        checkErr(err, "convertTreeAddresses ERROR");

        return 1;
};


int runCLConvertAddressesToGPU()
{
       ...
       tree_d = clCreateBuffer(context, CL_MEM_READ_WRITE, 30000 * sizeof(FttOct), NULL, NULL);
       root_d = clCreateBuffer(context, CL_MEM_READ_WRITE, number_of_roots * sizeof(FttRootCell), NULL, NULL);
       cellData_d = clCreateBuffer(context, CL_MEM_READ_WRITE, 30000 * (sizeof(struct Data_Of_Cell) + sizeof (GfsSolidVector)), NULL, NULL);
       boundary_d = clCreateBuffer(context, CL_MEM_READ_WRITE, 5 * sizeof(FttRootCell), NULL, NULL);
       //solid_data_d = clCreateBuffer(context, CL_MEM_READ_WRITE, numberOfCells * sizeof(GfsSolidVector), NULL, NULL);  
       size_d = clCreateBuffer(context, CL_MEM_READ_WRITE, 30 * sizeof(unsigned long), NULL, NULL); 
      
       global[0] = (size_t)12000;
       nd=1;           
      
       unsigned long treeh = (unsigned long)tree;
       unsigned long cellDatah = (unsigned long)cellData;
       unsigned long rooth = (unsigned long)Root;
       unsigned long boundaryh = (unsigned long)Boundary;
       unsigned long treehEnd = (unsigned long)treeEnd;
       unsigned long roothEnd = (unsigned long)RootEnd;
       unsigned long boundaryhEnd = (unsigned long)BoundaryEnd;
      
       FttOct *t = (FttOct *)tree;
       FttRootCell *r = (FttRootCell *) Root;

       printf("host: tree is %p ,", tree);
       printf("host: treeh is %ld, \n", treeh);
       printf("host: Root is %p, \n", Root);
       printf("host: Boundary is %p\n", Boundary); 
                 
       printf("host : Root[0].cell.children is %p\n", r[0].cell.children);
       printf("host : Root[0].cell.data is %p\n", r[0].cell.data);
       printf("host : Root[0].neighbors.c[0] is %p\n", r[0].neighbors.c[0]);
       printf("host : Root[0].neighbors.c[1] is %p\n", r[0].neighbors.c[1]);
       printf("host : Root[0].neighbors.c[2] is %p\n", r[0].neighbors.c[2]);
       printf("host : Root[0].neighbors.c[3] is %p\n", r[0].neighbors.c[3]);

       for(i = 0; i < number_of_roots; i++)
         printf("host : Root[%d] is %p\n", i, r + i);

       printf("host : cellData is %p\n", cellData);
 
       FttOct * treeOct = (FttOct *)tree;
       GfsSolidVector * solid;
       
       for(j = 0; j < FTT_CELLS; j++) 
         for(i = 0; i < FTT_CELLS; i++) {
            printf("tree[%d].cell[%d].children = %p, ", j, i, treeOct[j].cell[i].children);
            printf("tree[%d].neighbors.c[%d] = %p\n", j, i, treeOct[j].neighbors.c[i]);
            printf("tree[%d].cell[%d].data = %p, ", j, i, treeOct[j].cell[i].data);
            
            if(treeOct[j].cell[i].data !=NULL)
             printf("st -> solid is %p\n", GFS_STATE(&(treeOct[j].cell[i]))->solid);
         } 
            
       clSetKernelArg(convertTreeAddresses_kernel, 0, sizeof(cl_mem), &tree_d);
       clSetKernelArg(convertTreeAddresses_kernel, 1, sizeof(cl_mem), &cellData_d);
       clSetKernelArg(convertTreeAddresses_kernel, 2, sizeof(cl_mem), &root_d);
       clSetKernelArg(convertTreeAddresses_kernel, 3, sizeof(cl_mem), &boundary_d);
       clSetKernelArg(convertTreeAddresses_kernel, 4, sizeof(unsigned long), &treeh);
       clSetKernelArg(convertTreeAddresses_kernel, 5, sizeof(unsigned long), &cellDatah);
       clSetKernelArg(convertTreeAddresses_kernel, 6, sizeof(unsigned long), &rooth);
       clSetKernelArg(convertTreeAddresses_kernel, 7, sizeof(unsigned long), &boundaryh);
       clSetKernelArg(convertTreeAddresses_kernel, 8, sizeof(unsigned long), &treehEnd);
       clSetKernelArg(convertTreeAddresses_kernel, 9, sizeof(unsigned long), &roothEnd);
       clSetKernelArg(convertTreeAddresses_kernel, 10, sizeof(unsigned long), &boundaryhEnd);
       clSetKernelArg(convertTreeAddresses_kernel, 11, sizeof(int), &number_of_roots);
       
       err = clEnqueueWriteBuffer(commands, tree_d , CL_TRUE, 0, 30000 * sizeof(struct _FttOct), tree, 0, NULL, NULL);
       checkErr(err, "tree_d WRITE BUFFER ERROR");   
       err = clEnqueueWriteBuffer(commands, root_d , CL_TRUE, 0, number_of_roots * sizeof(FttRootCell), Root, 0, NULL, NULL);
       checkErr(err, "root_d WRITE BUFFER ERROR"); 
       err = clEnqueueWriteBuffer(commands, cellData_d, CL_TRUE, 0, 30000 * (sizeof(struct Data_Of_Cell) + sizeof (GfsSolidVector)), cellData, 0, NULL, NULL);
       checkErr(err, "cellData_d WRITE BUFFER ERROR");
       err = clEnqueueWriteBuffer(commands, boundary_d , CL_TRUE, 0, 5 * sizeof(FttRootCell), Boundary, 0, NULL, NULL);
       checkErr(err, "boundary_d WRITE BUFFER ERROR");
       
       err = clEnqueueNDRangeKernel(commands, convertTreeAddresses_kernel, nd, NULL, global, NULL, 0, NULL, NULL);
       checkErr(err, "NDRange cell_traverse_level_non_leafs ERROR"); 
      

       printf("host : sizeof(FttOct) = %ld\n", sizeof(struct _FttOct));
       printf("host : sizeof(FttOct *) = %ld\n", sizeof(struct _FttOct *));
       printf("host : sizeof(struct FttCell) = %ld\n", sizeof(struct _FttCell));
       printf("host : sizeof(struct FttCell *) = %ld\n", sizeof(struct _FttCell *));
       printf("host : sizeof(struct _GfsStateVector) = %ld\n", sizeof(struct _GfsStateVector));
       printf("host : sizeof(struct _GfsStateVector *) = %ld\n", sizeof(struct _GfsStateVector *));
       printf("host : sizeof(struct _GfsFaceStateVector) = %ld\n", sizeof(struct _GfsFaceStateVector));
       printf("host : sizeof(struct _FttCellNeighbors) = %ld\n", sizeof( struct _FttCellNeighbors));
       printf("host : sizeof(struct _FttVector) = %ld\n", sizeof(struct _FttVector));
       printf("host : sizeof(FttRootCell) = %ld\n", sizeof(FttRootCell));
       printf("host : sizeof(gdouble) =  %ld\n", sizeof(gdouble));
       printf("host : sizeof(guint) =  %ld\n", sizeof(guint));


       printf("host differences\n");
      
       printf("######Host: GfsStateVector########\n");
       GfsStateVector a;
		     ...
		     gpointer data)
{
  ...
  #ifndef CONTIGUOUS_ALLOCATION
  oct = g_malloc0 (sizeof (FttOct));
  #else
  oct = my_malloc_tree(sizeof (FttOct));
  numberOcts++;
  #endif
			    ...
			    gpointer init_data)
{
  ...
  printf("FTT_ROOT_CELL(root)->neighbors.c[d] %p, d is %d\n", FTT_ROOT_CELL(root)->neighbors.c[d], d);
  ...
  printf("FTT_ROOT_CELL(root)->neighbors.c[od] %p, od is %d\n", FTT_ROOT_CELL(root)->neighbors.c[od], od);  
				  ...
				  gpointer init_data)
{
  ...
  printf("ftt_cell_set_neighbor_match: neighbor is %p \n", neighbor);
  printf("ftt_cell_set_neighbor_match: root is %p \n", root);

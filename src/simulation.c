...
int cell_func_times = 0;

void cell_func(int *msg)
{
  cell_func_times++;
  //printf("%s\n", msg);
}

void check_tree(GfsDomain * domain)
{
  printf("#############CHECK   TREE ##########################\n"); 
  gfs_domain_cell_traverse(domain, FTT_PRE_ORDER, FTT_TRAVERSE_ALL, -1, (FttCellTraverseFunc)cell_func, &msg);
}
//------------------------

unsigned long long a_rdtsc_t = 0;
unsigned long long b_rdtsc_t = 0;
unsigned long long gfs_centered_velocity_advection_diffusion_rdtsc = 0;
unsigned long long gfs_advance_tracers_rdtsc = 0;

double gfs_centered_velocity_adv_diff_clk = 0.0;
double gfs_domain_face_traverse1_clk =0.0;
double gfs_domain_face_traverse2_clk =0.0;
double gfs_predicted_face_velocities_clk = 0.0;
double gfs_mac_projection_clk = 0.0;
double gfs_approximate_projection_clk = 0.0;
double gfs_source_coriolis_implicit_clk = 0.0;
double gfs_correct_centered_velocities_clk = 0.0;
double gfs_domain_cell_traverse_sim_run_clk = 0.0;
double gfs_advance_tracers_clk = 0.0;
double gfs_update_gradients_clk = 0.0;
double gfs_event_do_clk = 0.0;
double gfs_event_half_do_clk = 0.0;
double gts_range_add_value_clk = 0.0;
double gfs_simulation_adapt_clk = 0.0;
double gfs_simulation_run_start_clk = 0.0;
double gfs_domain_variable_centered_sources_adv_trc_clk = 0.0;
double gfs_domain_cell_traverse_adv_trc_clk = 0.0;
double gfs_tracer_advection_diffusion_adv_trc_clk = 0.0;
double gfs_variables_swap_clk = 0.0;
...
cl_kernel cell_traverse_level_non_leafs_kernel;
cl_mem ftt_opposite_direction_d;
cl_mem coords_d;
cl_mem index_d;

/*int ftt_opposite_direction_array[FTT_NEIGHBORS] = {1, 0, 3, 2};//mallon dn xreiazetai pali dhlwsh

double coords[FTT_CELLS * 3] = {-1., 1., 0., 1., 1., 0., -1., -1., 0., 1., -1., 0.};

unsigned int index_array[FTT_NEIGHBORS_2D * FTT_CELLS/2] = {1, 3, 0, 2, 0, 1, 2, 3};
*/

unsigned long sizes[30];

int runCLTraverseTreePerLevel(unsigned int max_depth, GfsVariable * v, unsigned int i)
{ 
       int err;
       ...
       if(first_time) {
          if(context == NULL) 
             printf("poisson context is NULL\n"); 
          if(commands == NULL) 
             printf("poisson commands is NULL");
          if(program == NULL) 
	     printf("poisson program is NULL"); 	
         
         /*ftt_opposite_direction_d = clCreateBuffer(context, CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR, FTT_NEIGHBORS * sizeof(int),  ftt_opposite_direction_array, 0);
         coords_d = clCreateBuffer(context, CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR, FTT_CELLS * 3 * sizeof(double),  coords, 0);
         index_d = clCreateBuffer(context, CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR, FTT_NEIGHBORS_2D * FTT_CELLS/2 * sizeof(double),  index_array, 0) ;*/
         
         err = clSetKernelArg(cell_traverse_level_non_leafs_kernel, 2, sizeof(void *), (void *)&root_d);
         checkErr(err, "cell_traverse_level_non_leafs_kernel root_d ERROR");
         err = clSetKernelArg(cell_traverse_level_non_leafs_kernel, 3, sizeof(void *), (void *)&tree_d);
         checkErr(err, "cell_traverse_level_non_leafs_kernel tree_d ERROR");
         err = clSetKernelArg(cell_traverse_level_non_leafs_kernel, 6, sizeof(void *), (void *)&size_d);
         checkErr(err, "cell_traverse_level_non_leafs_kernel root_d ERROR");
         
        /*err = clSetKernelArg(cell_traverse_level_non_leafs_kernel, 5, sizeof(cl_mem), &ftt_opposite_direction_d);
         checkErr(err, "cell_traverse_level_non_leafs_kernel ftt_opposite_direction_d ERROR");
         err = clSetKernelArg(cell_traverse_level_non_leafs_kernel, 6, sizeof(cl_mem), &coords_d);
         checkErr(err, "cell_traverse_level_non_leafs_kernel coords_d ERROR");
         err = clSetKernelArg(cell_traverse_level_non_leafs_kernel, 7, sizeof(cl_mem), &index_d);
         checkErr(err, "cell_traverse_level_non_leafs_kernel index_d ERROR");*/
         
         //first_time = 0;
       }
      
       err = clSetKernelArg(cell_traverse_level_non_leafs_kernel, 0, sizeof(cl_uint), &max_depth);
       checkErr(err, "cell_traverse_level_non_leafs_kernel max depth ERROR");
       err = clSetKernelArg(cell_traverse_level_non_leafs_kernel, 1, sizeof(cl_uint), &i);
       checkErr(err, "cell_traverse_level_non_leafs_kernel i ERROR");
       err = clSetKernelArg(cell_traverse_level_non_leafs_kernel, 4, sizeof(cl_int), &(v->i));
       checkErr(err, "cell_traverse_level_non_leafs_kernel v_d ERROR");  
       err = clSetKernelArg(cell_traverse_level_non_leafs_kernel, 5, sizeof(cl_char), &first_time);
       checkErr(err, "cell_traverse_level_non_leafs_kernel first_time ERROR"); 
       ...
       /*err = clEnqueueWriteBuffer(commands, cellData_d, CL_TRUE, 0, 43688 * (sizeof(struct Data_Of_Cell) + sizeof (GfsSolidVector)), cellData, 0, NULL, NULL);
       checkErr(err, "cell_data WRITE BUFFER ERROR");*/ 

       err = clEnqueueNDRangeKernel(commands, cell_traverse_level_non_leafs_kernel, nd, NULL, global, NULL, 0, NULL, NULL);
       checkErr(err, "NDRange cell_traverse_level_non_leafs ERROR"); 
       if(first_time) {
         err = clEnqueueReadBuffer(commands, size_d, CL_TRUE, 0, 30 * sizeof (unsigned long), sizes, 0, NULL, NULL);
         checkErr(err, "cell_data READ BUFFER ERROR:");
       ...
       /*err = clEnqueueReadBuffer(commands, cellData_d, CL_TRUE, 0, 43688 * (sizeof(struct Data_Of_Cell) + sizeof (GfsSolidVector)), cellData, 0, NULL, NULL);
       checkErr(err, "cell_data READ BUFFER ERROR:");*/
       first_time = 0;
       
      /*clReleaseMemObject(ftt_opposite_direction_d);
       clReleaseMemObject(coords_d);
       clReleaseMemObject(index_d);*/
       return 1;
};
#endif
...
void gfs_advance_tracers (GfsSimulation * sim, gdouble dt)
{
  ...
  while (i) {
    if (GFS_IS_VARIABLE_TRACER_VOF (i->data)) {
      ...
      gfs_domain_variable_centered_sources_adv_trc_clk = gfs_domain_variable_centered_sources_adv_trc_clk +( stop.tv_sec - start.tv_sec )
      ...
      if( clock_gettime( CLOCK_REALTIME, &stop) == -1 ) {
       ...
       gfs_tracer_advection_diffusion_adv_trc_clk = gfs_tracer_advection_diffusion_adv_trc_clk  +( stop.tv_sec - start.tv_sec )
          + ( stop.tv_nsec - start.tv_nsec )/ BILLION;
      ...
      if( clock_gettime( CLOCK_REALTIME, &stop) == -1 ) {
       ...
       gfs_domain_cell_traverse_adv_trc_clk = gfs_domain_cell_traverse_adv_trc_clk +( stop.tv_sec - start.tv_sec )
          + ( stop.tv_nsec - start.tv_nsec )/ BILLION; 
    }
    i = i->next;
  }  
...
static void simulation_run (GfsSimulation * sim)
{
 ...
 /* unsigned long cpuMask;
  ...
  gfs_simulation_run_start_clk = gfs_simulation_run_start_clk +( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/ (double)BILLION;  
 
  #ifdef CONTIGUOUS_ALLOCATION
   if(runCLConvertAddressesToGPU()) printf("\nrunCLConvertAddressesToGPU()\n\n");
  #endif

  check_tree(domain);
  printf("cell_func was called %d times\n", cell_func_times);
  cell_func_times = 0;

  printf("numberOcts is %d\n\n", numberOcts);
  ...
  if (sim->time.i == 0) {
   ...
   if( clock_gettime( CLOCK_REALTIME, &stop) == -1 ) {
    ...
    gfs_approximate_projection_clk +=( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/ (double)BILLION;
    ...
    gfs_advance_tracers_clk += ( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/ (double)BILLION;
  }
    ...
    gfs_update_gradients_clk +=( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/(double) BILLION;
  ...
  while (sim->time.t < sim->time.end &&
    ...
    if( clock_gettime( CLOCK_REALTIME, &stop) == -1 ) {
     ...
     gfs_event_do_clk +=( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/ (double)BILLION;
    if (sim->advection_params.linear) {
     ...
     gfs_domain_face_traverse1_clk += ( stop.tv_sec - start.tv_sec )
          +(double) ( stop.tv_nsec - start.tv_nsec )/(double) BILLION;
     ...
     gfs_domain_face_traverse2_clk += ( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/(double)BILLION;
    }
    else {
      if( clock_gettime( CLOCK_REALTIME, &start) == -1 ) {
       ...
       gfs_predicted_face_velocities_clk += ( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/(double) BILLION;
    }  
    ...
    gfs_variables_swap_clk += ( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/ (double)BILLION;
    ...
    gfs_mac_projection_clk += ( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/ (double)BILLION;
    ...
    gfs_event_half_do_clk += ( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/(double) BILLION;
    ...
    gfs_centered_velocity_adv_diff_clk += ( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/(double) BILLION;

    b_rdtsc_t = rdtsc();
    gfs_centered_velocity_advection_diffusion_rdtsc += b_rdtsc_t - a_rdtsc_t;
    ...
    if (gc) {
      ...
      gfs_source_coriolis_implicit_clk += ( stop.tv_sec - start.tv_sec )
          +(double) ( stop.tv_nsec - start.tv_nsec )/ (double)BILLION;
      ...
      gfs_correct_centered_velocities_clk += (stop.tv_sec - start.tv_sec )
          +(double) ( stop.tv_nsec - start.tv_nsec )/ (double)BILLION;
    }
    ...
    gfs_domain_cell_traverse_sim_run_clk +=( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/(double)BILLION;
    ...
    gfs_simulation_adapt_clk += (stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/(double) BILLION;
    ...
    b_rdtsc_t = rdtsc();
    gfs_advance_tracers_rdtsc += b_rdtsc_t - a_rdtsc_t;
    gfs_advance_tracers_clk += ( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/(double) BILLION;
    ...
    gts_range_add_value_clk =  gts_range_add_value_clk +( stop.tv_sec - start.tv_sec )
          + (double)( stop.tv_nsec - start.tv_nsec )/ (double)BILLION; 

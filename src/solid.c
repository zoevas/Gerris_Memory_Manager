						  ...
						  GfsGenericSurface * s)
{
  ...
  case 2: {
	...
	GFS_STATE (cell)->solid = solid = g_malloc0 (sizeof (GfsSolidVector));
        #else
         #ifdef WAY1
          GFS_STATE (cell)->solid= solid = my_malloc_cell_data(sizeof (GfsSolidVector));
          #ifdef DEBUG_MODE
          printf("1. solid.c:  GFS_STATE (cell)->solid is %p\n",  GFS_STATE (cell)->solid);
          #endif
         #endif
        #endif
     }
					      ...
					      InitSolidParams * p)
{
  ...
  if (!solid) {
	...
	GFS_STATE (cell)->solid = solid = g_malloc0 (sizeof (GfsSolidVector));
        ...
        GFS_STATE (cell)->solid = solid = my_malloc_cell_data(sizeof (GfsSolidVector));
        #ifdef DEBUG_MODE
        printf("2. solid.c:  GFS_STATE (cell)->solid is %p\n",  GFS_STATE (cell)->solid);
        #endif
       #endif
    #endif
  }
...
void gfs_cell_init_solid_fractions_from_children (FttCell * cell)
{
  ...
  if (cell_is_mixed) {
	...
	GFS_STATE (cell)->solid = solid = g_malloc0 (sizeof (GfsSolidVector));
         ...
         GFS_STATE (cell)->solid = solid = my_malloc_cell_data(sizeof (GfsSolidVector));
         #ifdef DEBUG_MODE
         printf("3. solid.c:  GFS_STATE (cell)->solid is %p\n",  GFS_STATE (cell)->solid);
         #endif
        #endif
      #endif    
    }
...
void gfs_solid_coarse_fine (FttCell * parent, GfsDomain * domain)
{
  ...
  for (i = 0; i < FTT_CELLS; i++) {
	...
	GfsSolidVector * s = GFS_STATE (child.c[i])->solid = g_malloc (sizeof (GfsSolidVector));
      #else
         #ifdef WAY1
          GfsSolidVector * s = GFS_STATE (child.c[i])->solid = my_malloc_cell_data(sizeof (GfsSolidVector));
          #ifdef DEBUG_MODE
          printf("4. solid.c:  s is %p\n",  s);
          #endif
         #endif
      #endif

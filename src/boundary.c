...
gpointer RootEnd, Root;
gpointer BoundaryEnd, Boundary;

gpointer my_malloc_roots(unsigned long size)
{
       static gpointer nextRootFree;
       gpointer returnPointer;

	if (Root == NULL) {
		Root = g_malloc0(size);
		RootEnd = (gpointer)((unsigned long)Root + size);
		nextRootFree = Root;
                return NULL;
	}
	
	returnPointer = nextRootFree;
        nextRootFree = (gpointer)((unsigned long)nextRootFree + size);
	if ((unsigned long)nextRootFree >= (unsigned long)RootEnd) {
		printf("Roots:Memory allocation problem\n");
		exit(0);
	}
	return(returnPointer);
};

FttCell * my_malloc_init_roots(FttCellInitFunc init, gpointer data)
{
      FttCell *cell;

      cell = my_malloc_roots(sizeof (struct _FttRootCell));
      if (init)
        (* init) (cell, data);
...
gpointer my_malloc_boundaries(unsigned long size)
{
       static gpointer nextBoundaryFree;
       gpointer returnPointer;

	if (Boundary == NULL) {
		Boundary = g_malloc0(size);
		BoundaryEnd = (gpointer)((unsigned long)Boundary + size);
		nextBoundaryFree = Boundary;
                return NULL;
	}
	
	returnPointer = nextBoundaryFree;
        nextBoundaryFree = (gpointer)((unsigned long)nextBoundaryFree + size);
	if ((unsigned long)nextBoundaryFree >= (unsigned long)BoundaryEnd) {
		printf("Boundary:Memory allocation problem\n");
		exit(0);
	}
	return(returnPointer);
};

FttCell * my_malloc_init_boundaries(FttCellInitFunc init, gpointer data)
{
      FttCell *cell;

      cell = my_malloc_boundaries(sizeof (struct _FttRootCell));
      if (init)
        (* init) (cell, data);
...
static void match (FttCell * cell, GfsBoundary * boundary)
{
  ...
  if (ftt_cell_level (neighbor) == level) {
	...
	GFS_STATE (cell)->solid = g_malloc0 (sizeof (GfsSolidVector));
        #else
         #ifdef WAY1
          GFS_STATE (cell)->solid = my_malloc_cell_data(sizeof (GfsSolidVector));
          #ifdef DEBUG_MODE
           printf("match:  GFS_STATE (cell)->solid is %p\n",  GFS_STATE (cell)->solid);
          #endif
         #endif
        #endif        
      }
...
static void boundary_match (GfsBoundary * boundary)
{
  if (boundary->root == NULL) {
    ...
    #else
    boundary->root = my_malloc_init_boundaries((FttCellInitFunc) gfs_cell_init, domain);
    #endif
    printf("boundary_match: boundary->root is %p\n", boundary->root);
				...
				FttDirection d)
{
  ...
  printf("gfs_boundary_new: boundary->root is %p \n", boundary->root);
  boundary->box = box;
  box->neighbor[d] = GTS_OBJECT (boundary);
  printf("gfs_boundary_new: box->neighbor[%d] is %p, d is %d\n", d, box->neighbor[d], d);
...
void gfs_gedge_link_boxes (GfsGEdge * edge)
{
  ...
  if (edge->rotate < FTT_NEIGHBORS) {
    ...
    if (GFS_BOX (periodic) == b2) {
      ...
      printf("GTS_OBJECT b1->root is %p\n", b1->root);
      printf("GTS_OBJECT b2->root is %p\n", b2->root);
			  ...
			  FttDirection d)
{
  ...
  printf("gfs_gedge_new: b1->root is %p , b2->root is %p\n", b1->root, b2->root);
  edge->d = d;
...
static void gfs_box_read (GtsObject ** o, GtsFile * fp)
{
  ...
  #else
  b->root = my_malloc_init_roots((FttCellInitFunc) gfs_cell_init, domain);
  #endif
  printf("gfs_box_read: b->root is %p\n", b->root);
  ...
  if (fp->type == '{') {
	...
	if (GFS_IS_BOUNDARY (b->neighbor[d])) {
	  GfsBoundary * boundary = GFS_BOUNDARY (b->neighbor[d]);
          printf("gfs_box_read: boundary->root is %p\n", boundary->root);
